    ...
    <div className="rounded-xl glass p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-semibold text-white/90">
          {lang === 'vi' ? 'So sánh trước / sau thay đổi' : 'Before / after comparison'}
        </p>
        <select
          value={selectedId}
        ...
        >
          {OPTIONS.map((c) => (
            <option key={c.id} value={c.id}>
              {formatDate(c.date)} · Block {c.block} · {c.description[lang].slice(0, 40)}...
            </option>
          ))}
        </select>
      </div>
      ...
      {change && (
        <>
          <p className="mb-3 text-xs text-white/45">{change.description[lang]}</p>
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex h-40 items-center justify-center rounded-lg border border-navy-700 bg-navy-850 text-white/35">
                <div className="flex flex-col items-center gap-1.5">
                  <ImageOff size={22} />
                  <span className="text-[11px]">{lang === 'vi' ? 'Trước thay đổi (minh hoạ)' : 'Before change (illustrative)'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1">
              <div className="flex h-40 items-center justify-center rounded-lg border border-status-success/40 bg-status-success/5 text-white/35">
                <div className="flex flex-col items-center gap-1.5">
                  <ImageOff size={22} />
                  <span className="text-[11px]">{lang === 'vi' ? 'Sau thay đổi (minh hoạ)' : 'After change (illustrative)'}</span>
                </div>
              </div>
            </div>
          </div>
