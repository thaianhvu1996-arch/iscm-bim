    ...
    <div className="glass shrink-0 rounded-2xl px-6 py-3">
      <div className="mb-1.5 flex items-center justify-between">
        <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-white/45">
          <PlayCircle size={13} /> {lang === 'vi' ? 'Tiến độ thi công (mô phỏng 4D)' : 'Construction progress (4D simulation)'}
        </p>
        <p className="text-sm font-semibold text-brand">
          {lang === 'vi' ? 'Tháng' : 'Month'} {Math.min(9, Math.max(1, Math.round(month)))}/9 ·{' '}
          {formatMonthShort(addMonths(PROJECT_START, Math.round(month) - 1), lang)}
        </p>
      </div>
      <input
        type="range"
