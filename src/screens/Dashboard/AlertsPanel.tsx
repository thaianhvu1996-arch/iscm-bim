...
export function AlertsPanel() {
  ...
  return (
    <div className="flex h-full flex-col rounded-xl glass p-4">
      <p className="mb-3 text-sm font-semibold text-white/90">Cảnh báo cần xử lý</p>
      {visibleAlerts.length === 0 ? (
        <p className="text-sm text-white/45">Không có cảnh báo nào.</p>
      ) : (
          ...
          {visibleAlerts.map((a) => {
            ...
            return (
              <div key={a.id} className="rounded-lg border border-navy-700 bg-navy-850 p-3">
                ...
                <p className="text-xs leading-relaxed text-white/75">{a.title}</p>
                <p className="mt-1.5 text-[11px] text-white/45">Phụ trách: {a.assignee}</p>
              </div>
            )
          })}
        </div>
